\begin{abstract}%摘要+摘要+摘要+摘要+摘要
	...
	在数据口径层面，周度面板\texttt{data/processed/dwts\_weekly\_panel.csv}与赛季特征\texttt{data/processed/dwts\_season\_features.csv}的整体缺失率分别为\textbf{4.54\%}与\textbf{2.42\%}（以空值/NaN占全表单元格比例计），为后续Q1--Q4提供了稳定、可审计的输入边界。
	...
	在Percent机制口径下，Q1覆盖\textbf{335}个赛季-周点（其中\textbf{266}周存在淘汰/退赛约束），有效样本量占比（ESS ratio）的均值为\textbf{0.627}、第10百分位为\textbf{0.404}；证据强度（evidence）的均值为\textbf{0.320}、第10百分位为\textbf{0.069}，用于识别“低可识别/高风险周”并支撑后续反事实核查。
	...
	在34个赛季的对比中，有\textbf{22}个赛季出现Percent与Rank的淘汰差异周（\texttt{diff\_weeks\_percent\_vs\_rank}>0），差异周数的中位数为\textbf{1}、最大值为\textbf{5}，用于证明“机制差异并非仅由极端个例驱动”。
	%（第3段）	对于问题2我们用。。。。。。。.
	...
	在主线设定\texttt{alpha=0.5}、\texttt{sigma\_scale=2.0}与压力强度\texttt{outlier\_mult}$\in\{2,5,10\}$下，推荐机制汇总表\texttt{outputs/tables/paper\_q4\_recommendation\_summary.csv}给出一致的推荐：\textbf{Rank}机制的技术保护（TPI）均值为\textbf{0.849}（95\%区间[0.829, 0.869]），观众表达（Fan）均值为\textbf{0.393}（95\%区间[0.335, 0.451]）；其鲁棒性失效率（Fail）在三档压力下分别为\textbf{0.440/0.456/0.460}，用于量化“离群动员”对制度稳定性的影响幅度。
	...
	\noindent\textbf{Keywords}: DWTS, latent fan vote inference, counterfactual mechanism evaluation, mixed-effects interpretability, robustness stress tests
